...
Server-managed workers record adapter capabilities with each runtime. Non-writable adapters such as Copilot are normalized to resume-command mode, session/export identifiers are captured from stdout and stderr, and `send_worker_input` uses stdin or resume delivery according to the recorded capability. `get_diagnostic_report` summarizes runtime input state, fallback handoff candidates, tool errors, and closeout blockers.
